...
// Runtime NN inference (no training here)
...
class _GamePageState extends State<GamePage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadAi() async {
    try {
      final js = await rootBundle.loadString('assets/ai/policy.json');
      setState(() {
  ...
  void _onTick(Duration _) {
    ...
    setState(() {
      // AI control
      if (aiPlay && _ai != null && _terrain != null && _worldSize != null) {
        try {
          final (thA, leftA, rightA) = _ai!.act(
            lander: lander,
            terrain: _terrain!,
      ...
      for (int i = _particles.length - 1; i >= 0; i--) {
        ...
        p.life -= dt * 1.8;
        p.vel += Offset(0, t.gravity * 0.05 * 0.2);
        p.pos += p.vel * dt;
      ...
      builder: (context, constraints) {
        ...
        final stack = Stack(
          children: [
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        // AI Play toggle (disabled until policy is loaded)
                        IconButton.filledTonal(
                          ...
                          icon: Icon(aiPlay ? Icons.smart_toy : Icons.smart_toy_outlined),
                          tooltip: _ai == null ? 'AI policy not loaded' : (aiPlay ? 'Stop AI' : 'AI Play'),
                        ),
                      ],
                    ),
            ...
            // Controls (disabled while AI plays)
            _buildControls(),
            ...
            // AI status chip (now under a Scaffold -> has Material ancestor)
            Positioned(
              top: 8,
              right: 8,
              child: Chip(
                label: Text(_ai == null ? 'AI: not loaded' : (aiPlay ? 'AI: playing' : 'AI: idle')),
                avatar: Icon(Icons.smart_toy, size: 16, color: aiPlay ? Colors.greenAccent : Colors.white70),
        ...
        // ✅ Wrap in Scaffold to provide a Material ancestor (fixes Chip error)
        return Scaffold(
  ...
  Widget _buildControls() {
    return IgnorePointer(
      ignoring: status != GameStatus.playing || aiPlay,
      child: Align(
...
class GamePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    try {
      ...
      // Debug aid: avoid the red overlay and log the error
      debugPrint('Painter error: $e\n$st');
    }
  }
  ...
  void _paintPad(Canvas canvas) {
    double x1 = terrain.padX1;
    double x2 = terrain.padX2;
    final y = terrain.padY;
    if (!x1.isFinite || !x2.isFinite || !y.isFinite) return;
    if (x2 < x1) { final t = x1; x1 = x2; x2 = t; }
    final w = x2 - x1;
    if (w <= 0) return;

    final padRect = Rect.fromLTWH(x1, y - 4, w, 8);
    final paint = Paint()
  ...
  bool shouldRepaint(covariant GamePainter oldDelegate) {
    return oldDelegate.lander != lander ||
        oldDelegate.terrain != terrain ||
        oldDelegate.thrusting != thrusting ||
